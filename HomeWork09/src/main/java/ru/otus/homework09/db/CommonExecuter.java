...
import ru.otus.homework09.db.handlers.IORMResultSetHandler;
import ru.otus.homework09.db.handlers.ISimpleResultSetHandler;
...
public class CommonExecuter {
    ...
    public void execSelect(String query, ISimpleResultSetHandler handler) throws SQLException {
        try (Statement stmt = connection.createStatement()){
            stmt.execute(query);
            try (ResultSet result = stmt.getResultSet()) {
